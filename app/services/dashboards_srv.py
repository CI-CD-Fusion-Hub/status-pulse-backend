...
from app.utils.enums import SessionAttributes, DashboardScopes
...
class DashboardService:
    ...
    async def get_all(self, request: Request):
        ...
        dashboards_endpoints = [
            DashboardOut(
                ...
                endpoints=[DashboardEndpoint(id=e.endpoint.id, name=e.endpoint.name, url=e.endpoint.url,
    ...
    async def get_by_id(self, request: Request, dashboard_id: int):
        ...
        dashboard_endpoints.endpoints = [DashboardEndpoint(id=e.endpoint.id, name=e.endpoint.name, url=e.endpoint.url,
                                                           status=e.endpoint.status.status)
    ...
    async def create_dashboard(self, request: Request, dashboard_data: CreateDashboard):
        try:
            ...
            await self._upsert_endpoints_to_dashboard(request, dashboard.id, dashboard_data.endpoints)
    ...
    async def _upsert_endpoints_to_dashboard(self, request: Request, dashboard_id: int, endpoints_ids: List[int]):
        user_endpoints = request.session.get(SessionAttributes.USER_ENDPOINTS_PERM.value)

        for endpoint_id in endpoints_ids:
            if endpoint_id not in user_endpoints.keys():
                raise CustomHTTPException(detail=f"Dashboard have been created but but "
                                                 f"endpoint with id {endpoint_id} does not exist.",
                                          status_code=status.HTTP_404_NOT_FOUND)

        await self.dashboards_dao.delete_assigned_endpoints(dashboard_id)
        await self.dashboards_dao.add_endpoint_to_dashboard(dashboard_id, endpoints_ids)
